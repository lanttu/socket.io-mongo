...
describe('Mongo', function () {
    describe('tests subscribing and unsubscribing', function () {
        ...
        it('should only receive events not send by self', function (next) {
            a.subscribe('noself', function (arg) {
                arg.should.equal('foo');
                next();
            });
            
            a.publish('noself', 'bar');
            b.publish('noself', 'foo');
            
        });

        it('should receive subscribed event before unsubscribe', function (next) {
            a.subscribe('before', function (arg) {
                arg.should.equal('foobar');
                a.unsubscribe('before', function () {
                    b.publish('before', 'fail');
                    next();
                });
            });
            b.publish('before', 'foobar');
        });
        
        it('should publish to multiple subscribers but not self', function (next) {
            var messages = 0;
            function subscription(arg1, arg2, arg3) {
                ...
                if (messages === 2) return next();
            }
            a.subscribe('multi', function () {
                // Should receive 0
                throw new Error('Should not trigger');
            });
            b.subscribe('multi', subscription); // Should receive 2
            c.subscribe('multi', subscription); // Should receive 2

            setTimeout(function () {
                // a.publish('multi', 'foo', 'bar', 'baz');
                // a.publish('multi', 'foo', 'bar', 'baz');
                a.publish('multi', 'foo', 'bar', 'baz');
            }, 100);
        });
